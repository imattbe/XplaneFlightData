...
//
// Compile: g++ -std=c++20 -O3 -o density_altitude_calculator
// density_altitude_calculator.cpp
//
// Usage: ./density_altitude_calculator <pressure_alt_ft> <oat_celsius>
// <ias_kts> <tas_kts> [force_error]
...
// DA = PA + [120 * (OAT - ISA)]
Float64 calculate_density_altitude(Float64 pressure_altitude_ft,
                                   Float64 oat_celsius) {
  ...
  Float64 density_altitude =
      pressure_altitude_ft + (density_alt_factor * temp_deviation);
...
// σ = ρ / ρ₀
Float64 calculate_density_ratio(Float64 pressure_altitude_ft,
                                Float64 oat_celsius) {
  ...
  Float64 pressure_ratio =
      pow(1.0 - pressure_altitude_constant * pressure_altitude_ft,
...
DensityAltitudeData
calculate_density_altitude_data(Float64 pressure_altitude_ft,
                                Float64 oat_celsius, Float64 ias_kts,
                                Float64 tas_kts) {
  DensityAltitudeData result;

  result.pressure_altitude_ft = pressure_altitude_ft;
  result.density_altitude_ft =
      calculate_density_altitude(pressure_altitude_ft, oat_celsius);
  ...
  // Air density ratio
  result.air_density_ratio =
      calculate_density_ratio(pressure_altitude_ft, oat_celsius);
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 6) {
    force_exception =
        (std::strcmp(argv[5], "1") == 0 || std::strcmp(argv[5], "true") == 0);
  }

  if (force_exception) {
    return error_simulated;
    // throw std::runtime_error("CRITICAL: Required dataref
    // 'sim/weather/isa_deviation' not found in X-Plane API");
  }
